...
import { getEpisode, findExportDir, runCommand, parseEpisodeNumber, EXPORTS_DIR, WORKSPACE_ROOT } from "./helpers.js";
...
async function main() {
  ...
  const script = path.join(WORKSPACE_ROOT, "scripts", "src", "export-video.ts");
  ...
  runCommand(path.join(WORKSPACE_ROOT, "scripts", "node_modules", ".bin", "tsx"), [script], {
