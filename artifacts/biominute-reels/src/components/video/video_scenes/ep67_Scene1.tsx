...
import { Coffee, Zap, Leaf } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">Core Benefits Are Equal</span>
        </motion.div>
        {[
          { icon: Coffee, color: '#f97316', title: 'Benefits come from the coffee', body: 'Caffeine and antioxidants come from the bean itself — milk doesn\'t change that' },
          { icon: Zap, color: '#10b981', title: 'Caffeine is caffeine', body: 'Adding milk doesn\'t reduce caffeine content or blunt its alertness effects' },
          { icon: Leaf, color: '#f97316', title: 'Antioxidants survive the milk', body: 'Chlorogenic acids and polyphenols remain active whether you add milk or not' },
        ].map((item, i) => {
