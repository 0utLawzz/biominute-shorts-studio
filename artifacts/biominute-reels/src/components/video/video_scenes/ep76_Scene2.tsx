...
import { AlertTriangle, Timer, Dumbbell } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">When Recovery Is Too Short</span>
        </motion.div>
        {[
          { icon: AlertTriangle, color: '#f97316', title: 'Fatigue keeps piling up', body: 'If you keep training hard without enough recovery, soreness and performance drop can linger' },
          { icon: Timer, color: '#2F6FED', title: 'Progress can stall', body: 'Adaptation depends on balancing stress and rest, not just adding more sessions' },
          { icon: Dumbbell, color: '#f97316', title: 'More is not always better', body: 'Easy days, mobility, or a full rest day can keep your next workout higher quality' },
        ].map((item, i) => {
