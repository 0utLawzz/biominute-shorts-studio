    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#2F6FED]/10 border border-[#2F6FED]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#2F6FED] font-display font-bold text-[22px] uppercase tracking-wider">Why It Happens</span>
        </motion.div>
        {[
          { icon: AlertCircle, color: '#f97316', label: 'Not a broken metabolism', body: 'Metabolism rarely "breaks" — it adapts, but invisible calorie gaps are the real cause' },
          { icon: Search, color: '#10b981', label: 'Stress raises hunger hormones', body: 'Cortisol drives cravings for calorie-dense foods and disrupts appetite signals' },
          { icon: BookOpen, color: '#2F6FED', label: 'One week of honest tracking', body: 'Research: tracking without judgment for 7 days reveals the real intake pattern' },
        ].map((item, i) => {
