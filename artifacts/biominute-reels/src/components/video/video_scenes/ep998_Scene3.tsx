...
import { CheckCircle2, Film, Circle } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">Export Stage</span>
        </motion.div>
        {[
          { icon: CheckCircle2, color: '#10b981', label: 'Playwright recorded', body: 'Browser captured the full scene loop at 1080×1920' },
          { icon: CheckCircle2, color: '#10b981', label: 'ffmpeg mixed audio', body: 'Background music looped under the video track' },
          { icon: Film, color: '#f97316', label: 'MP4 ready', body: 'Output file is verified at the correct vertical resolution' },
        ].map((item, i) => {
