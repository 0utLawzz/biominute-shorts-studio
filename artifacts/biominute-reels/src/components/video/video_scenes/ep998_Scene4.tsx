    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">Publish Stage</span>
        </motion.div>
        {[
          { icon: CheckCircle2, color: '#10b981', label: 'OAuth ready', body: 'Refresh token exchanged and YouTube API client authenticated' },
          { icon: Upload, color: '#f97316', label: 'Uploading', body: 'Video bytes sent to YouTube with title and description' },
          { icon: Circle, color: '#2F6FED', label: 'Public check', body: 'Confirm the video is live and the DB records the video ID' },
        ].map((item, i) => {
