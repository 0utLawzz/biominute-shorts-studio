...
import { Coffee, Beaker, AlertCircle } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">What Decaf Really Means</span>
        </motion.div>
        {[
          { icon: Beaker, color: '#10b981', title: 'Most caffeine is removed', body: 'Decaf removes ~97% of caffeine — but not every last molecule' },
          { icon: Coffee, color: '#f97316', title: 'Still 2–15 mg per cup', body: 'A typical decaf cup has a small fraction compared to ~95 mg in regular coffee' },
          { icon: AlertCircle, color: '#2F6FED', title: 'Sensitive? It still counts', body: 'If you are highly caffeine-sensitive or pregnant, even this small amount can add up' },
        ].map((item, i) => {
