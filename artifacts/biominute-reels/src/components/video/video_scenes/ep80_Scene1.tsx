...
import { BellRing, Moon, EyeOff } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#7c3aed]/10 border border-[#7c3aed]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#7c3aed] font-display font-bold text-[22px] uppercase tracking-wider">Why the Bedside Phone Is a Problem</span>
        </motion.div>
        {[
          { icon: BellRing, color: '#7c3aed', title: 'Notifications wake your brain', body: 'Even silent buzzes, banners, or flashes can keep your mind on alert' },
          { icon: Moon, color: '#2F6FED', title: 'Light can delay melatonin', body: 'A glowing screen signals wake time and can make it harder to feel sleepy' },
          { icon: EyeOff, color: '#7c3aed', title: 'Scrolling keeps you up', body: 'The bigger issue is often the behavior: one more check turns into more wake time' },
        ].map((item, i) => {
