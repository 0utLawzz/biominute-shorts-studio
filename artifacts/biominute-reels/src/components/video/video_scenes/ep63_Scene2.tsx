...
import { Utensils, Dumbbell, Apple } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">The Real Drivers</span>
        </motion.div>
        {[
          { icon: Utensils, color: '#f97316', label: 'Overall diet pattern', body: 'What you eat consistently across the day matters far more than any single drink' },
          { icon: Dumbbell, color: '#10b981', label: 'Activity and movement', body: 'Regular activity is one of the strongest predictors of sustainable weight change' },
          { icon: Apple, color: '#2F6FED', label: 'Calorie balance', body: 'A modest appetite dip from vinegar can\'t overcome a large calorie surplus' },
        ].map((item, i) => {
