...
import { Moon, ShieldCheck, Clock } from 'lucide-react';
  ...
  return (
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        ...
        {[
          { icon: Clock, color: '#10b981', label: 'Same bedtime every night', body: 'A consistent schedule anchors your circadian rhythm and deepens sleep stages' },
          { icon: Moon, color: '#7c3aed', label: 'Dark, cool room (18–20 °C)', body: 'Core body temperature drop triggers deep sleep — a cool room accelerates this' },
          { icon: ShieldCheck, color: '#2F6FED', label: 'If tired persists — get tested', body: 'Persistent fatigue after 8 hours warrants a sleep study to rule out apnea' },
        ].map((item, i) => {
