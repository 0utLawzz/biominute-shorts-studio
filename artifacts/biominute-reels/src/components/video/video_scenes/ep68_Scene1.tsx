...
import { Sunrise, Clock, RefreshCw } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">Your Circadian Clock</span>
        </motion.div>
        {[
          { icon: Clock, color: '#f97316', title: 'Your body has an internal clock', body: 'The circadian rhythm is a 24-hour biological cycle governing sleep, hormones, and energy' },
          { icon: Sunrise, color: '#10b981', title: 'Wake time anchors the clock', body: 'Consistent wake time is the strongest signal for setting your circadian rhythm each day' },
          { icon: RefreshCw, color: '#f97316', title: 'More reliable than sleep time', body: 'When you wake matters more to your clock than when you fall asleep — wake time anchors both' },
        ].map((item, i) => {
