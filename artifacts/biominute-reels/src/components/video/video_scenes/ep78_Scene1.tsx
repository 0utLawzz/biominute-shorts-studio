...
import { BrainCircuit, Eye, SlidersHorizontal } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#7c3aed]/10 border border-[#7c3aed]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#7c3aed] font-display font-bold text-[22px] uppercase tracking-wider">Counting Is Not the Best Distraction</span>
        </motion.div>
        {[
          { icon: BrainCircuit, color: '#7c3aed', title: 'Imagery works better', body: 'A vivid mental scene gives your brain something engaging and calming to do' },
          { icon: Eye, color: '#2F6FED', title: 'Counting is too boring', body: 'Repetitive counting can leave too much room for worry to creep back in' },
          { icon: SlidersHorizontal, color: '#7c3aed', title: 'Gentle focus beats effort', body: 'The goal is not to force sleep, but to occupy attention just enough to relax' },
        ].map((item, i) => {
