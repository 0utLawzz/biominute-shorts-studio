    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#7c3aed]/10 border border-[#7c3aed]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#7c3aed] font-display font-bold text-[22px] uppercase tracking-wider">The Real Risk Is Behavior</span>
        </motion.div>
        {[
          { icon: ShieldCheck, color: '#7c3aed', title: 'It is not special radiation', body: 'The problem is usually light, alerts, and the temptation to keep using the device' },
          { icon: PhoneOff, color: '#2F6FED', title: 'Silence helps, but distance helps more', body: 'Do not rely only on do-not-disturb — move the phone away from the pillow' },
          { icon: Zap, color: '#7c3aed', title: 'Protect the wind-down window', body: 'Make the last part of the night calm so your brain connects bed with sleep, not stimulation' },
        ].map((item, i) => {
