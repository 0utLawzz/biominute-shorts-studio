...
import { FlaskConical, TrendingDown, Flame } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">The Breakfast Myth</span>
        </motion.div>
        {[
          { icon: FlaskConical, color: '#f97316', title: 'Marketing, not metabolism', body: 'The "starvation mode" idea came more from food advertising than actual metabolic research' },
          { icon: TrendingDown, color: '#10b981', title: 'No meaningful RMR change', body: 'Studies comparing skippers vs. eaters find no significant difference in resting metabolic rate' },
          { icon: Flame, color: '#f97316', title: 'Metabolism stays active', body: 'Your body continues burning calories at baseline whether you eat breakfast or not' },
        ].map((item, i) => {
