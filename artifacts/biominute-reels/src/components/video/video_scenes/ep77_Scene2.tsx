...
import { MonitorUp, AlertTriangle, MoveVertical } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">Why It Can Disappoint</span>
        </motion.div>
        {[
          { icon: MonitorUp, color: '#f97316', title: 'A desk does not undo long sitting', body: 'If you stand all day but never move, you still miss the benefits of regular activity breaks' },
          { icon: AlertTriangle, color: '#2F6FED', title: 'Static posture can feel bad', body: 'Prolonged standing may cause foot, leg, or back discomfort for some people' },
          { icon: MoveVertical, color: '#f97316', title: 'The real fix is variety', body: 'Switching between sitting, standing, and moving is more realistic for health and comfort' },
        ].map((item, i) => {
      ...
      <div className="absolute w-full px-12 text-center z-20" style={{ bottom: BOTTOM_SAFE_ZONE_PX + 80 }}>
        <motion.h2 className="text-[#f8fafc] text-[46px] font-bold uppercase tracking-wider font-display leading-tight"
          ...
          Standing Alone
          <motion.span className="text-[#f97316] block mt-2"
