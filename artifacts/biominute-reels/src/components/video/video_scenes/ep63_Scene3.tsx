...
import { ShieldCheck, Droplets, AlertCircle } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">Safe If You Like It</span>
        </motion.div>
        {[
          { icon: Droplets, color: '#10b981', label: 'Dilute it in water', body: 'A splash in water before meals is generally safe for most healthy adults' },
          { icon: ShieldCheck, color: '#2F6FED', label: 'Don\'t overdo it', body: 'Large amounts or undiluted vinegar can irritate teeth and stomach lining' },
          { icon: AlertCircle, color: '#f97316', label: 'No dramatic results', body: 'Enjoy it if you like the taste, but keep expectations realistic' },
        ].map((item, i) => {
