...
import { Wind, Heart, Footprints } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#2F6FED]/10 border border-[#2F6FED]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#2F6FED] font-display font-bold text-[22px] uppercase tracking-wider">Sweat Is Cooling</span>
        </motion.div>
        {[
          { icon: Wind, color: '#2F6FED', title: 'Sweating helps cool you', body: 'Your body sweats to regulate temperature — it is not a direct measure of workout quality' },
          { icon: Heart, color: '#f97316', title: 'Heart and lungs still work', body: 'A brisk walk can raise heart rate and improve cardiovascular fitness without drenching you' },
          { icon: Footprints, color: '#7c3aed', title: 'Effort matters more', body: 'What counts is intensity, duration, and consistency — not how wet your shirt gets' },
        ].map((item, i) => {
