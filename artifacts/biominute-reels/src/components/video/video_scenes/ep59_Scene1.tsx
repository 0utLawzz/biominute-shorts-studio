...
import { Hand, ShieldCheck, Activity } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#2F6FED]/10 border border-[#2F6FED]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#2F6FED] font-display font-bold text-[22px] uppercase tracking-wider">What the Research Shows</span>
        </motion.div>
        {[
          { icon: ShieldCheck, color: '#10b981', title: 'No arthritis link found', body: 'Studies comparing habitual crackers to non-crackers show no difference in arthritis rates' },
          { icon: Hand, color: '#2F6FED', title: 'It\'s a gas bubble pop', body: 'The sound is cavitation — gas bubbles collapsing in synovial fluid, not bone grinding' },
          { icon: Activity, color: '#f97316', title: 'Might cause mild swelling', body: 'Very heavy, lifelong cracking may slightly affect grip strength — but not arthritis' },
        ].map((item, i) => {
