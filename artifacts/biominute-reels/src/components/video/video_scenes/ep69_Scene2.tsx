    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">The Reality Check</span>
        </motion.div>
        {[
          { icon: Scale, color: '#f97316', label: 'The boost is modest', body: 'Cold showers increase calorie burn by a small, short-lived amount — not enough to move the scale alone' },
          { icon: AlertTriangle, color: '#10b981', label: 'Not a weight-loss tool', body: 'No study shows cold showers producing meaningful fat loss independent of diet and exercise' },
          { icon: TrendingUp, color: '#f97316', label: 'Effect fades with adaptation', body: 'Your body adapts to cold over time, reducing the thermogenic response with regular exposure' },
        ].map((item, i) => {
