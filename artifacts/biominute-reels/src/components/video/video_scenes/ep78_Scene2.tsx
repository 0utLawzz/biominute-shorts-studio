    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#2F6FED]/10 border border-[#2F6FED]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#2F6FED] font-display font-bold text-[22px] uppercase tracking-wider">A Better Bedtime Trick</span>
        </motion.div>
        {[
          { icon: Sparkles, color: '#2F6FED', title: 'Detailed imagery occupies the mind', body: 'A calm mental scene is more engaging than bland counting and can crowd out anxious thoughts' },
          { icon: Brain, color: '#7c3aed', title: 'Less effort, more relaxation', body: 'You are not trying to solve anything — just gently redirect attention away from arousal' },
          { icon: Moon, color: '#2F6FED', title: 'Use something soothing', body: 'Pick a familiar place or scene that feels safe, slow, and easy to picture' },
        ].map((item, i) => {
