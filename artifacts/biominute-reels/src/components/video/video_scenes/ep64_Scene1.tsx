...
import { Dumbbell, Heart, ShieldCheck } from 'lucide-react';
  ...
  return (
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        ...
        {[
          { icon: Heart, color: '#10b981', title: 'Generally fine for moderate exercise', body: 'Most healthy people can do light-to-moderate workouts fasted without harm' },
          { icon: Dumbbell, color: '#2F6FED', title: 'Personal preference matters', body: 'Some people feel better fasted; others feel weak — both are valid' },
          { icon: ShieldCheck, color: '#f97316', title: 'Not inherently bad', body: 'Fasted training does not damage muscle or metabolism for typical workouts' },
        ].map((item, i) => {
