...
import { Search, Candy, BrainCircuit } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">What the Research Says</span>
        </motion.div>
        {[
          { icon: Search, color: '#10b981', title: 'No consistent link found', body: 'Multiple studies, including double-blind trials, have failed to confirm sugar causes hyperactivity' },
          { icon: Candy, color: '#f97316', title: 'Sugar is metabolically neutral here', body: 'The body processes sugar the same way at parties as at home — context is what changes' },
          { icon: BrainCircuit, color: '#2F6FED', title: 'JAMA meta-analysis agrees', body: 'Wolraich et al. (1995) found no reliable effect of sugar on children\'s behavior or cognition' },
        ].map((item, i) => {
