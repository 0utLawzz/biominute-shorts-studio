...
import { Thermometer, Snowflake, BedDouble } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#7c3aed]/10 border border-[#7c3aed]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#7c3aed] font-display font-bold text-[22px] uppercase tracking-wider">Temperature Matters</span>
        </motion.div>
        {[
          { icon: Thermometer, color: '#2F6FED', title: 'Your body cools to sleep', body: 'A drop in core temperature is part of the normal sleep signal' },
          { icon: Snowflake, color: '#7c3aed', title: 'A cooler room helps that signal', body: 'Many people sleep better when the bedroom is cool but comfortable' },
          { icon: BedDouble, color: '#2F6FED', title: 'Too warm works against you', body: 'Heat can make it harder to relax, fall asleep, and stay asleep' },
        ].map((item, i) => {
