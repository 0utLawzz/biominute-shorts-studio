    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">How Bloating Works</span>
        </motion.div>
        {[
          { icon: Activity, color: '#10b981', label: 'It\'s gas, not fat', body: 'Bloating is gas moving through your gut — diet, pace, and bacteria drive it, not weight' },
          { icon: AlertCircle, color: '#f97316', label: 'Food sensitivities vary', body: 'Lactose, fructose, or FODMAPs may trigger bloating in sensitive individuals specifically' },
          { icon: Zap, color: '#2F6FED', label: 'Stress slows digestion', body: 'The gut–brain axis means anxiety and stress can stall gut motility and cause bloating' },
        ].map((item, i) => {
