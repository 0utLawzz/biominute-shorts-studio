    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">What This Tests</span>
        </motion.div>
        {[
          { icon: Upload, color: '#10b981', title: 'Upload now', body: 'The video is uploaded to YouTube as a private scheduled post' },
          { icon: Clock, color: '#f97316', title: 'Wait unattended', body: 'YouTube will flip the video public at the scheduled time without our server' },
          { icon: ShieldCheck, color: '#2F6FED', title: 'Confirm later', body: 'Check that the video is public and the DB status is updated' },
        ].map((item, i) => {
