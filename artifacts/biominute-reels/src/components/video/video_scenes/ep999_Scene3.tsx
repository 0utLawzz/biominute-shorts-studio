...
import { ServerOff, Globe, Database } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">Why This Works</span>
        </motion.div>
        {[
          { icon: ServerOff, color: '#10b981', label: 'No server needed', body: 'YouTube stores the private video and publishes it at the scheduled time' },
          { icon: Globe, color: '#f97316', label: 'YouTube owns timing', body: 'The platform clock flips privacyStatus to public at publishAt' },
          { icon: Database, color: '#2F6FED', label: 'DB records the ID', body: 'We save youtube_video_id so the scheduler never re-uploads' },
        ].map((item, i) => {
