...
import { HeartPulse, TrendingUp, Apple } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">Post-Meal Movement</span>
        </motion.div>
        {[
          { icon: Apple, color: '#f97316', title: 'After dinner can be powerful', body: 'Dinner is often larger, so a short walk after it can help blunt a bigger glucose rise' },
          { icon: HeartPulse, color: '#2F6FED', title: 'Breakfast walks still help', body: 'Walking after breakfast also improves glucose handling and starts the day more active' },
          { icon: TrendingUp, color: '#7c3aed', title: 'Consistency matters most', body: 'The best timing is the one you can repeat most days, not a perfect clock time' },
        ].map((item, i) => {
