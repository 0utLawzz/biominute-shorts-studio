...
import { FileCode, Film, Upload } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">What This Tests</span>
        </motion.div>
        {[
          { icon: FileCode, color: '#f97316', title: 'Generate', body: 'Scene components are created and TypeScript compiles cleanly' },
          { icon: Film, color: '#10b981', title: 'Export', body: 'Playwright + ffmpeg render a valid 1080×1920 MP4 with background music' },
          { icon: Upload, color: '#2F6FED', title: 'Publish', body: 'YouTube upload completes and the video is publicly available' },
        ].map((item, i) => {
