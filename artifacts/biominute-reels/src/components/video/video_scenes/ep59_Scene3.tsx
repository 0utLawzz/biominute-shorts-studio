...
import { ShieldCheck, Hand, XCircle } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">The Verdict</span>
        </motion.div>
        {[
          { icon: ShieldCheck, color: '#10b981', label: 'Joints are not being damaged', body: 'Decades of evidence confirm no meaningful arthritis risk from knuckle cracking' },
          { icon: XCircle, color: '#f97316', label: 'It is annoying to others', body: 'The only real downside is social — it genuinely irritates people nearby' },
          { icon: Hand, color: '#2F6FED', label: 'Crack away if you like it', body: 'Unless it causes you pain, the habit is harmless from a joint health perspective' },
        ].map((item, i) => {
