...
import { Snowflake, BedDouble, Wind } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#2F6FED]/10 border border-[#2F6FED]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#2F6FED] font-display font-bold text-[22px] uppercase tracking-wider">Make the Bedroom Sleep-Friendly</span>
        </motion.div>
        {[
          { icon: Snowflake, color: '#2F6FED', title: 'Cool the room', body: 'A lower temperature can support the natural drop in body heat that helps sleep begin' },
          { icon: BedDouble, color: '#7c3aed', title: 'Use breathable bedding', body: 'Sheets and blankets that trap less heat can help you stay comfortable through the night' },
          { icon: Wind, color: '#2F6FED', title: 'Let air move', body: 'A fan or light airflow can improve comfort if your room tends to feel stuffy' },
        ].map((item, i) => {
