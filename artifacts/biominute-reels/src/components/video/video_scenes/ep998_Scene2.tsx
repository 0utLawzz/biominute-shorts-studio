    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">Checklist</span>
        </motion.div>
        {[
          { icon: CheckCircle2, color: '#10b981', label: 'Scene files created', body: 'All 6 scenes are in place and the active renderer imports them' },
          { icon: CheckCircle2, color: '#10b981', label: 'TypeScript clean', body: 'No type errors before the export starts' },
          { icon: Circle, color: '#f97316', label: 'MP4 exported', body: 'Waiting for ffmpeg to finish mixing audio and video' },
        ].map((item, i) => {
