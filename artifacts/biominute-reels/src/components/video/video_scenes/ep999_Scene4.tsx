...
import { CalendarCheck, Eye, MessageCircle } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">Verification</span>
        </motion.div>
        {[
          { icon: CalendarCheck, color: '#10b981', label: 'Schedule set', body: 'publishAt is a future UTC timestamp in the YouTube status object' },
          { icon: Eye, color: '#f97316', label: 'Watch for it', body: 'After the publish time, the video should appear publicly' },
          { icon: MessageCircle, color: '#2F6FED', label: 'CTA', body: 'Comment below when you see this go live' },
        ].map((item, i) => {
