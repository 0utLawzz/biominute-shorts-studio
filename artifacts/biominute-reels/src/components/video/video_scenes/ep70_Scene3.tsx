...
import { Clock, ShieldCheck, Sunrise } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">The Simple Fix</span>
        </motion.div>
        {[
          { icon: Clock, color: '#f97316', label: 'Delay by 15–30 minutes', body: 'A short buffer between waking and checking in lets your brain ease into alertness naturally' },
          { icon: Sunrise, color: '#10b981', label: 'Start with something calm', body: 'Stretching, water, or a few quiet minutes before screens keeps your morning tone in your control' },
          { icon: ShieldCheck, color: '#f97316', label: 'Low-stakes content is fine', body: 'Not all morning phone use is harmful — calm content or checking the weather doesn\'t spike stress' },
        ].map((item, i) => {
