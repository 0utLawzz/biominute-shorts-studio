...
import { ShieldCheck, Clock3, Footprints } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">Make It Stick</span>
        </motion.div>
        {[
          { icon: Footprints, color: '#10b981', label: 'Choose a repeatable window', body: 'Pick breakfast, dinner, or both — the best time is the one you will actually keep' },
          { icon: Clock3, color: '#2F6FED', label: 'Keep it light and steady', body: 'A short consistent walk is easier to maintain than a huge one you skip later' },
          { icon: ShieldCheck, color: '#f97316', label: 'Any movement beats none', body: 'If you already sit a lot, a small post-meal stroll is still a meaningful upgrade' },
        ].map((item, i) => {
