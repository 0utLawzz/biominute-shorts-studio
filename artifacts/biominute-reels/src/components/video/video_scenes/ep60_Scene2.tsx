...
import { TrendingDown, BrainCircuit, AlertTriangle } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#2F6FED]/10 border border-[#2F6FED]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#2F6FED] font-display font-bold text-[22px] uppercase tracking-wider">What Catch-Up Misses</span>
        </motion.div>
        {[
          { icon: TrendingDown, color: '#f97316', label: 'Insulin resistance persists', body: 'Even with weekend recovery, blood sugar regulation stays impaired after 5 days of poor sleep' },
          { icon: BrainCircuit, color: '#7c3aed', label: 'Memory consolidation gaps', body: 'Learning and memory encoded during sleep debt week is lost — catch-up can\'t restore it' },
          { icon: AlertTriangle, color: '#2F6FED', label: 'Social jet lag costs extra', body: 'The shift between weekday/weekend sleep times adds its own circadian disruption to the debt' },
        ].map((item, i) => {
      ...
      <div className="absolute w-full px-12 text-center z-20" style={{ bottom: BOTTOM_SAFE_ZONE_PX + 80 }}>
        <motion.h2 className="text-[#f8fafc] text-[46px] font-bold uppercase tracking-wider font-display leading-tight"
          ...
          The Debt
          <motion.span className="text-[#2F6FED] block mt-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.4, ...SPRING_SNAPPY }}>
            Stays
          </motion.span>
        </motion.h2>
      </div>
