    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#2F6FED]/10 border border-[#2F6FED]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#2F6FED] font-display font-bold text-[22px] uppercase tracking-wider">Recovery Builds Adaptation</span>
        </motion.div>
        {[
          { icon: HeartPulse, color: '#2F6FED', title: 'Recovery is where gains happen', body: 'Training creates the stimulus, but rest lets muscles, tendons, and the nervous system adapt' },
          { icon: ShieldCheck, color: '#f97316', title: 'Rest can prevent injuries', body: 'Too little recovery stacks fatigue and raises the chance that small aches turn into bigger setbacks' },
          { icon: ArrowRightLeft, color: '#2F6FED', title: 'Light movement still counts', body: 'Walking, mobility work, and easy cycling can support circulation without turning every day into a hard session' },
        ].map((item, i) => {
