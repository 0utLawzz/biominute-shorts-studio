...
import { Clock3, Salad, HeartPulse } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">Brisk Walking Counts</span>
        </motion.div>
        {[
          { icon: HeartPulse, color: '#2F6FED', label: 'Heart health benefits show up fast', body: 'A brisk walk after a period of sitting can still raise heart rate and improve circulation' },
          { icon: Salad, color: '#f97316', label: 'Post-meal walks smooth glucose', body: 'Light walking after eating can blunt the blood sugar spike from a larger meal' },
          { icon: Clock3, color: '#7c3aed', label: 'Consistency beats the clock', body: 'Whether it is after breakfast or dinner, the routine matters more than the exact meal timing' },
        ].map((item, i) => {
