...
import { Eye, Coffee, Moon } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">The Hidden Gaps</span>
        </motion.div>
        {[
          { icon: Eye, color: '#10b981', title: 'Portions are deceiving', body: 'Studies show people underestimate intake by 20–40% — even trained dietitians' },
          { icon: Coffee, color: '#f97316', title: 'Liquid calories add up', body: 'Drinks, sauces, and "healthy" snacks can hide hundreds of calories daily' },
          { icon: Moon, color: '#2F6FED', title: 'Weekend eating undoes weekdays', body: 'Two relaxed days can erase a full week of careful eating if not tracked' },
        ].map((item, i) => {
