    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">Upload Phase</span>
        </motion.div>
        {[
          { icon: CheckCircle2, color: '#10b981', label: 'Video uploaded', body: 'MP4 sent to YouTube and held as private' },
          { icon: Lock, color: '#f97316', label: 'Privacy status', body: 'privacyStatus=private with a future publishAt timestamp' },
          { icon: Timer, color: '#2F6FED', label: 'Timer set', body: 'YouTube will auto-publish at the scheduled UTC time' },
        ].map((item, i) => {
