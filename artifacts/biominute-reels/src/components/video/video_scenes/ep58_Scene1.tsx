...
import { Wind, Coffee, Apple } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">Common Triggers</span>
        </motion.div>
        {[
          { icon: Wind, color: '#10b981', title: 'Eating too fast', body: 'Swallowing air and gulping food leads directly to gas build-up in the gut' },
          { icon: Coffee, color: '#f97316', title: 'Carbonated drinks', body: 'Bubbles that go in must come out — CO₂ inflates the stomach quickly' },
          { icon: Apple, color: '#2F6FED', title: 'Sudden fibre spike', body: 'Adding high-fibre foods too fast overwhelms gut bacteria — introduce them gradually' },
        ].map((item, i) => {
