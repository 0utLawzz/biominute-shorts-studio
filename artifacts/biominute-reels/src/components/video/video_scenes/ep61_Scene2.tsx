...
import { BarChart3, Coffee, Zap } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">Caffeine Comparison</span>
        </motion.div>
        {[
          { icon: Coffee, color: '#f97316', label: 'Regular coffee', body: 'About 95 mg of caffeine per 8 oz cup — the baseline most people compare against' },
          { icon: BarChart3, color: '#10b981', label: 'Decaf coffee', body: 'Only 2–15 mg per cup — roughly 5–15% of a regular cup' },
          { icon: Zap, color: '#2F6FED', label: 'Most people won\'t notice', body: 'For the average adult, that small amount is below the perceptible threshold' },
        ].map((item, i) => {
