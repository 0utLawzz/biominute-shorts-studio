...
import { MoonStar, TimerReset, Salad } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#2F6FED]/10 border border-[#2F6FED]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#2F6FED] font-display font-bold text-[22px] uppercase tracking-wider">Why It Works</span>
        </motion.div>
        {[
          { icon: Salad, color: '#2F6FED', label: 'Short walks use glucose', body: 'Even 10 minutes of light movement after eating can help muscles take up glucose' },
          { icon: TimerReset, color: '#f97316', label: 'Meal size matters too', body: 'A larger dinner often creates a bigger spike, so a post-dinner walk can be especially useful' },
          { icon: MoonStar, color: '#7c3aed', label: 'The pattern matters most', body: 'Regular post-meal walks beat one perfect session when it comes to long-term habits' },
        ].map((item, i) => {
