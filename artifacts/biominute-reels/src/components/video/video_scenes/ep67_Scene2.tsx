    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">Where the Difference Lies</span>
        </motion.div>
        {[
          { icon: Flame, color: '#f97316', label: 'The extras add up fast', body: 'Sugar and cream can push a zero-calorie drink past 200 calories per cup' },
          { icon: Droplets, color: '#10b981', label: 'Milk adds small benefits', body: 'A splash of milk adds protein, calcium, and a little fat — not insignificant' },
          { icon: AlertTriangle, color: '#f97316', label: 'Watch the add-ins, not the coffee', body: 'Flavoured syrups and whipped cream are the real calorie culprits, not the milk itself' },
        ].map((item, i) => {
