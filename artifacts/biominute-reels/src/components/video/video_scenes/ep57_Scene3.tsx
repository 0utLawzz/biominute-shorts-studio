    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">The Real Fix</span>
        </motion.div>
        {[
          { icon: BookOpen, color: '#10b981', label: 'Track for 7 days, no judgment', body: 'Log everything honestly — it reveals where the invisible calories are coming from' },
          { icon: Scale, color: '#2F6FED', label: 'Weigh food, don\'t eyeball it', body: 'Visual portions are notoriously inaccurate — a kitchen scale changes the game' },
          { icon: ShieldCheck, color: '#f97316', label: 'Broken metabolism is rare', body: 'The science is clear: consistent tracking solves most "unexplained" weight plateaus' },
        ].map((item, i) => {
