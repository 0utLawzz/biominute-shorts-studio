...
import { FlaskConical, TrendingDown, AlertTriangle } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">What the Evidence Shows</span>
        </motion.div>
        {[
          { icon: FlaskConical, color: '#10b981', title: 'Small effect on appetite', body: 'Acetic acid may slightly blunt appetite and post-meal blood sugar spikes for some people' },
          { icon: TrendingDown, color: '#f97316', title: 'Weight loss is minor', body: 'Any effect is modest — it is not a substitute for diet and activity changes' },
          { icon: AlertTriangle, color: '#2F6FED', title: 'No magic fat burner', body: 'Studies show modest weight trends at best, not dramatic body composition changes' },
        ].map((item, i) => {
