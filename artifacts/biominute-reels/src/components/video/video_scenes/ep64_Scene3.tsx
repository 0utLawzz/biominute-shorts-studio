...
import { ShieldCheck, Banana, Timer } from 'lucide-react';
  ...
  return (
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        ...
        {[
          { icon: Timer, color: '#10b981', label: 'Short/easy: skip if you prefer', body: 'A light walk or 30-minute easy session is fine fasted for most people' },
          { icon: Banana, color: '#f97316', label: 'Long/hard: fuel beforehand', body: 'A banana or small snack 30–60 min before can protect performance' },
          { icon: ShieldCheck, color: '#2F6FED', label: 'No universal rule', body: 'It comes down to workout type, duration, and your personal tolerance' },
        ].map((item, i) => {
