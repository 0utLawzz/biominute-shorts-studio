...
import { Wind, HeartPulse, Dumbbell } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#2F6FED]/10 border border-[#2F6FED]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#2F6FED] font-display font-bold text-[22px] uppercase tracking-wider">Active Recovery Works Too</span>
        </motion.div>
        {[
          { icon: Wind, color: '#2F6FED', title: 'Gentle movement helps', body: 'Walking, stretching, or easy cycling can keep blood moving without a hard training load' },
          { icon: HeartPulse, color: '#f97316', title: 'Listen to your body', body: 'If you feel beat up, a lighter day can be the smarter move than forcing intensity' },
          { icon: Dumbbell, color: '#2F6FED', title: 'Consistency beats burnout', body: 'A sustainable plan with recovery keeps you training longer and progressing more reliably' },
        ].map((item, i) => {
