    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">Who Should Still Count It</span>
        </motion.div>
        {[
          { icon: Heart, color: '#10b981', label: 'Caffeine-sensitive people', body: 'Anxiety, jitters, or sleep issues can still be triggered by small amounts' },
          { icon: Baby, color: '#f97316', label: 'Pregnancy', body: 'Guidelines often include decaf in total caffeine limits because it is not zero' },
          { icon: ShieldCheck, color: '#2F6FED', label: 'Safe to enjoy, just be aware', body: 'Decaf is generally safe — the key is knowing it still contains trace caffeine' },
        ].map((item, i) => {
